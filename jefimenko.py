...
def get_E_obs(t_obs, t_delay, grid_coordinates, r_hat, r_diff_norm, factor):
    ...
    static_term = tmp[:, :, :, 4]
    E_obs_static, E_obs_rho, E_obs_J = np.zeros(3), np.zeros(3), np.zeros(3)

    for dim in range(3):
        E_obs_static[dim] = np.sum(factor * speed_of_light * r_hat[dim] *
                                   static_term / r_diff_norm)
        E_obs_rho[dim] = np.sum(factor * r_hat[dim] * rho_term)
        E_obs_J[dim] = -np.sum(factor * J_term[:, :, :, dim])

    return E_obs_static, E_obs_rho, E_obs_J
...
for i, r in enumerate(r_obs):
    ...
    # r_diff_norm is |r - r'| for each grid point
    r_diff_norm = delays[i] * speed_of_light
    factor = dV / (4 * np.pi * epsilon_0 * speed_of_light * r_diff_norm)
    r_hat = r_diff[i] / r_diff_norm

    # Helper function
    def get_E_obs_for_point(t_obs):
        return get_E_obs(t_obs, delays[i], grid_coordinates, r_hat,
                         r_diff_norm, factor)

    with Pool(args.np) as p:
        tmp = p.map(get_E_obs_for_point, t_obs, chunksize=1)
        E_obs_combined = np.array(tmp)
        E_obs_static, E_obs_rho, E_obs_J = E_obs_combined[:, 0], \
            E_obs_combined[:, 1], E_obs_combined[:, 2]

    # Save to csv file
    header = 't_obs,t_src,E_static_x,E_static_y,E_static_z,E_rho_x,'\
        'E_rho_y,E_rho_z,E_J_x,E_J_y,E_J_z'
    fname = file_prefix + f'_observer_{r[0]}_{r[1]}_{r[2]}.csv'
    all_data = np.array(
        [t_obs, t_obs-delays[i].mean(),
         E_obs_static[:, 0], E_obs_static[:, 1], E_obs_static[:, 2],
